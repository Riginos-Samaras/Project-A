...
void parser::printWeightsVector(){
    ...
    for (int count = 0; count < weights.size(); count++)                           //For Loop to Display All Products
            {
...
void parser::printDependenciesVector(){
    ...
    for (int count = 0; count < dependencies.size(); count++)                           //For Loop to Display All Products
            {
...
int first_flag=0; //to get the first token
    int num_of_data=0;
  ...
  // read each line of the file
   ...
   const char* token[MAX_TOKENS_PER_LINE] = {}; // initialize to 0
    ...
    num_of_data = atoi(token[0]); // cast the number of input data from char* to int
   ...
   const char* token1[MAX_TOKENS_PER_LINE] = {}; // initialize to 0
        token1[0] = strtok(buf2, DELIMITER); // first token
   ...
   const char* token2[MAX_TOKENS_PER_LINE] = {}; // initialize to 0
        token2[0] = strtok(buf3, ","); // first token
        token2[1] = strtok(0, ",");
